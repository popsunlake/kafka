...
Use `kafka-streams-groups.sh` to manage **Streams groups** for the Streams Rebalance Protocol (KIP‑1071): list and describe groups, inspect members and offsets/lag, reset or delete offsets for input topics, and delete groups (optionally including all internal topics).
...
**Use with care:** Mutating operations (offset resets/deletes, group deletion) affect how applications will reprocess data when restarted. Preview offset resets with `--dry-run` before using `--execute`, and ensure application instances are stopped/inactive and the group is empty before executing a mutating command.
...
Safety: 
    ...
    * With `--execute`, use `--delete-internal-topic <name>` to delete a specific internal topic, or `--delete-all-internal-topics` to delete all internal topics.
  * `--delete-offsets`: Delete offsets for `--all-input-topics` or one/more `--input-topic <name>` options.
...
## Common flags
  ...
  * `--dry-run`, `--execute`: Preview vs apply an offset reset; these options are only valid with `--reset-offsets`.
...
# Best practices and safety
  ...
  * Preview offset resets with `--dry-run` to verify topic scope and impact before `--execute`.
